...
func TestParseFlagsShortProfileForSubcommands(t *testing.T) {
	f, err := parseFlags([]string{"-p", "deepseek"})
	if err != nil || f.profile != "deepseek" {
		t.Errorf("parseFlags(-p deepseek) = %+v, %v", f, err)
	}
}
